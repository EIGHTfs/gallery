(() => {
  ...
  // 用户原话「增加收藏功能🩷，预设个不可删改的收藏分类，点了收藏的图片都会在里面平铺；登录用户可点收藏，游客可以看」
  ...
  let collapsedTreeNodes = new Set(); // 用户原话「侧边栏只能展开不能折叠」→ 手动折叠的侧边栏节点（key=rootPath|relPath）
  ...
  // 用户原话「标签页图标也替换顶部栏的◆这个图标」——favicon 同时更新浏览器标签和顶部栏 logo
  ...
  function updateAuthUI() {
    ...
    // 用户原话「登录之前设置按钮不显示，点击锁了输入正确密码，显示设置按钮。然后不再显示🔒的图片」
    if (!passwordSet) {
  ...
  async function init() {
    ...
    await loadAPIImages();
    restoreNav();
    pushNavHistory();  // 用户原话「顶部导航栏前后加< >实现后退前进」→ 初始状态入历史栈，导航后才能后退
  ...
  /* ─── 收藏功能（用户原话「增加收藏功能🩷，预设个不可删改的收藏分类，登录可点收藏，游客可看」）─── */
  // 规范化收藏 src：旧数据可能存了完整 URL（含端口），重启/换端口后裂开
  // 用户原话「重启过后，收藏下的图片裂开不能显示」→ 统一转成相对 /api/... 路径
  ...
  // 用户原话「图片路径明明有全路径，目录描述你只到根目录」→ 解析 file 参数补全
  ...
  async function toggleFavorite(p) {
    ...
    if (favoriteSrcs.has(src)) {
      ...
      // 用户原话「收藏里面的图片可以点开后描述来源路径」→ 保存 path/root
      const item = {
  ...
  // 用户原话「侧边栏只能展开不能折叠」→ 已展开(ancestor)点击=折叠不导航；未展开点击=展开+导航
  ...
  function currentScope() {
    ...
    // 用户原话「只想优化性能，不想改前端显示设计」→ statDir 从 O(N) 遍历 pool 改 O(1) 查表
    ...
    // 【改为】用户原话「只想优化性能，不想改前端显示设计」→ 查预计算表，显示逻辑不变
    ...
    // 用户原话「里面有单独图片，但这种情况下不影响，分组是里面有许多文件夹就分组」「怎么搞出3种情况了，明明都应该是同一种」
  ...
  function makeCard(p, opts = {}) {
    ...
    // 已收藏的图片显示 ❤️ 角标（用户原话「点了收藏的图片都会在里面平铺」）
  ...
  function render() {
    // 用户原话「增加收藏功能🩷，预设个不可删改的收藏分类，点了收藏的图片都会在里面平铺」
    ...
    // 游客分类（预设，用户原话「不登录只能上传到预设分类游客，项目本地实际存储」）
    if (activeFilter === "guest") {
      gallery.innerHTML = "";
      empty.style.display = "none";
      gallery.style.display = "";
      // 服务端 uploads/（持久）+ 本次会话前端上传（去重，按 src）
      const seen = new Set();
      const guestImgs = [];
    ...
    // 用户原话「分组折叠用的▾太小了改成▲▼。位置应该在后面，点后面▲▼才折叠展开，否则进入」
    ...
    groups.forEach(g => {
      ...
      if (!collapsed) {
        // 用户原话「分组中的分组也是分组，具体区分靠字符大小，外面那层字体大些」
        ...
        // 用户原话「分组中分组还是独占一行」→ group/leaf 独占一行直接挂 section；folder-card 放 wrap 横排
  ...
  // 轮播切换动画（用户原话「点开图片还是缺少过渡」→ 平滑滑动切换，不硬切）
  ...
  /* ─── Events ─── */
  // 用户原话「如果它已展开（是 ancestor），点击应该折叠它（而不是导航到它）。如果是未展开，点击就是展开它并导航到它」
  dirTree.addEventListener("click", e => {
    ...
    if (root === "all" || !hasKids) {
      ...
      if (root === "all") { activeDir = null; currentPath = ""; activeFilter = "all"; }
        ...
        currentPath = path;
        // 用户原话「选择收藏/游客分类后，不能通过左侧边栏跳转其他分类」→ 点侧边栏目录时切回该目录所属分类
  ...
  // ─── 屏蔽图片右键菜单（已按用户要求注释：允许长按/右键保存图片）───
  // document.addEventListener("contextmenu", e => {
  //   if (e.target.closest("img")) e.preventDefault();
  // }, true);

  // 用户原话「④下载功能并没有唤起浏览器下载图片，只是跳转到原图」
  ...
  // 用户原话「③点开图片之后，除了按钮切换图片，加上可以滑动切换」
  // 用户原话「点开图片之后，前后图片也都被点开，中间稍微有几厘米间距，滑动时前后图片一起滑动」
  ...
  /* ─── Add Category (inline edit box — 用户原话「分类的添加改成编辑框输入」) ─── */
  ...
  /* ─── Add Directory (本机目录浏览器 — 用户原话「添加目录改成📂读取本机目录选择，选择之后不需要弹窗口马上指定分类，直接默认指向all」) ─── */
  ...
  document.getElementById("addDirBtn").addEventListener("click", () => {
    ...
    li.querySelector(".db-select").addEventListener("click", () => {
      // 用户原话「直接默认指向all，作为里面的子文件夹。然后可以再分类」
  ...
  /* ─── Upload（上传区域搬到主页顶栏📤 + 全局拖拽，用户原话「上传区域搬到主页」）─── */
  ...
  async function handleFiles(files) {
    // 上传权限（用户原话「上传功能不能上传到收藏目录；不登录只能上传到预设分类游客」）
    if (activeFilter === "favorites") {
  ...
  /* ─── 卡片行预览滑动（用户原话「点开滑动确实实现了，但我其实想要的是没点开时那种的滑动」）
